...
v2/optimal-trading-filters-v2.tex.
...
and relative error.  Two independent machineries are used and cross-checked:
  ...
  * frequency domain: the Szego outer factor Phi(theta) = n_hat_+(i theta) via
    the Szego integral (eq:szego / eq:phi), a supplementary causality-gap
    quadrature (not stated in the current paper), and the closed-form rational
    factors (eq:exp-factor, eq:gp, eq:nv-factor);
...
Run:  python3 v2/experiments/test_all_results.py     (exit 0 iff all checks pass)
"""
...
def check_4_causality_gap():
    """Supplementary: power-law causality gap v/v_ant = sin(pi beta/2), theta-independent (not stated in the current paper)."""
    cases = []
    ok = True
    sigma2 = 1.0
    ...
    return ("4. Causality-gap identity  v/v_ant = sin(pi beta/2)  (supplementary; not in paper)", ok, cases)
...
def main():
    ...
    print("=" * 78)
    print("NUMERICAL VERIFICATION OF optimal-trading-filters-v2.tex")
    print("=" * 78)
    n_pass = 0
    results = []
    for chk in checks:
