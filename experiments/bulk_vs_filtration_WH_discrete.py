...
def kernel_full(K: int, c: float, gamma: float) -> np.ndarray:
    """Symmetric kernel of length 2K+1; index K is lag 0 = c (diagonal regularization)."""
    out = np.zeros(2*K+1)
    out[K] = c                       # G(0) regularization (any positive value)
...
def run(gamma: float = 0.4, rho: float = 0.9, c: float = 1.0,
    ...
    ker = kernel_full(K_hist, c, gamma)
    ...
    # So h* = M^{-1} R.
    GR = np.convolve(ker, np.array([rho ** abs(k) / (1 - rho ** 2)
                                    for k in range(-L_wiener, L_wiener + 1)]),
                     mode='same')
    # Center index of GR is len(GR)//2 corresponding to lag 0
    cen = len(GR) // 2
    M = np.array([[GR[cen + (i - j)] for j in range(L_wiener)] for i in range(L_wiener)])
    h_star = np.linalg.solve(M, R)
