...
*Origins.* Kolmogorov, Krein, Gohberg, Wiener; see [SK01] for a survey of methods and their interconnections.
...
*Origins.* Kalman 1960, Kalman–Bucy 1961; pedagogical statements in [Dev11] and connections to innovations in [Kai68].
...
The Kalman–Yakubovich–Popov (KYP) lemma — also called the positive-real lemma — is the formal equivalence linking three statements about a rational transfer matrix $G(z)$ ([And99] review; [MIT6.245] ch. 8 notes).
...
**Lindquist–Picci theorem (1979) [LP79].** The set of all minimal Markovian realisations of $y$ is in one-to-one correspondence with the set of all solutions $P$ of a certain *algebraic Riccati inequality*. The minimal element of this set gives the **forward innovations representation**; the maximal element gives the backward one. Both correspond to specific Wiener–Hopf factorisations of $\Phi$. See also [LMP95] for the zeros-of-spectral-factors / splitting-subspace dictionary.

**Consequence.** The steady-state Kalman filter — the recursive innovations form — is the time-domain realisation of the causal Wiener filter [Kai68; Dev11]. They are not approximations of each other; they are the same operator written in two notations.
...
2. Form the inner-outer / spectral factor $W(z) = C(zI - A)^{-1} K + L$, where $K, L$ are obtained from $P, B, D$ by an explicit linear formula (see [SK01] survey; [Var00]; [ZDG96] *Robust and Optimal Control*).
3. Then $W^*(z) W(z) = \Phi(z)$ on $|z|=1$, $W$ is causal-stable, and $W$ is the Wiener–Hopf causal factor up to a unitary on the right.

[Var00] emphasises a practical benefit: the ARE one solves has order equal to the (often small) McMillan degree of $G$, not the (potentially much larger) degree of $\Phi$.
...
This problem is **equivalent to a standard LQ stochastic control problem with a 2-D state**, and the closed form *is* the gain of a 2×2 discrete algebraic Riccati equation. The equivalence is general for Obizhaeva–Wang-type exponential-resilience execution problems ([BV22]; see also [Abe16] for an LQG framing of dynamic portfolio allocation with return predictability and price impact).
...
- The Riccati equation must become *operator-valued*. **Abi Jaber, Miller & Pham (2019) [AJMP19a]** establish existence and uniqueness of infinite-dimensional Riccati equations in $L^1(\mu \otimes \mu)$ — a space of signed matrix measures — for stochastic Volterra control problems. The companion paper [AJMP19b] gives the LQ control framework that subsumes the matrix-Volterra impact propagators of Abi Jaber–Neuman–Tuschmann.
...
Replacing the expected quadratic cost with a *worst-case* (H∞) cost replaces the $L^2$ inner product with an *indefinite* one (negative weights on the disturbance, positive on the error). The resulting estimator and controller satisfy a Riccati equation with **indefinite quadratic term** — geometrically, a Krein-space Riccati [HSK99].
...
All links checked via `web_search` results on 2026-05-31; no PDFs fetched (per workflow). HTML/landing pages preferred; one PDF link is cited from search metadata for the Tuel and MIT-notes references where no HTML version surfaced.

- [And73] B. D. O. Anderson. *Equivalence Relations for the Algebraic Riccati Equation.* SIAM J. Control 11(3) (1973). https://epubs.siam.org/doi/10.1137/0311022
- [And99] B. D. O. Anderson. *Old and New Perspectives on the Positive-real Lemma in Systems and Control* (GAMM 1998 plenary essay, pub. 1999). https://onlinelibrary.wiley.com/doi/10.1002/(SICI)1521-4001(199909)79:9%3C579::AID-ZAMM579%3E3.0.CO;2-8
- [AJMP19a] E. Abi Jaber, E. Miller, H. Pham. *Integral operator Riccati equations arising in stochastic Volterra control problems.* arXiv:1911.01903 (2019). https://arxiv.org/abs/1911.01903
- [AJMP19b] E. Abi Jaber, E. Miller, H. Pham. *Linear–Quadratic control for a class of stochastic Volterra equations: solvability and approximation.* arXiv:1911.01900 (2019). https://arxiv.org/abs/1911.01900
- [BV22] P. Bank, M. Voß. *Reducing Obizhaeva–Wang type trade execution problems to LQ stochastic control problems.* arXiv:2206.03772 (2022). https://arxiv.org/abs/2206.03772
- [Abe16] M. Abeille et al. *A generic solver for dynamic portfolio allocation problems when the market exhibits return predictability, price impact and partial observability* (the model is shown to fall into the LQG framework). arXiv:1611.00997 (2016); submitted by M. Abeille. https://arxiv.org/abs/1611.00997
- [Dev11] L. Devroye / H. P. Gavin. *Wiener and Kalman Filtering* notes (Duke / Devroye 2011). https://people.duke.edu/~hpgavin/SystemID/References/Devroye-Wiener+Kalman-2011.pdf
- [HSK99] B. Hassibi, A. H. Sayed, T. Kailath. *Indefinite-Quadratic Estimation and Control: A Unified Approach to H² and H∞ Theories.* SIAM monograph (1999). https://authors.library.caltech.edu/records/2h5wx-sj197
- [Kai68] T. Kailath. *An innovations approach to least-squares estimation — Part I.* IEEE Trans. Automat. Control AC-13 (1968); historical chapter at https://link.springer.com/chapter/10.1007/978-3-662-08546-2_5
- [LMP95] A. Lindquist, G. Michaletzky, G. Picci. *Zeros of Spectral Factors, the Geometry of Splitting Subspaces, and the Algebraic Riccati Inequality.* SIAM J. Control Optim. (1995). https://epubs.siam.org/doi/10.1137/S0363012992238667
- [LP79] A. Lindquist, G. Picci. *On the Stochastic Realization Problem.* SIAM J. Control Optim. 17(3) (1979). https://epubs.siam.org/doi/10.1137/0317028
- [Mar22] E. Martini et al. *Resolvent-based tools for optimal estimation and control via the Wiener–Hopf formalism.* arXiv:2201.00361 (2022). https://arxiv.org/abs/2201.00361
- [MIT6.245] *The KYP Lemma* (MIT 6.245 ch. 8 lecture notes). https://web.mit.edu/6.245/www/images/ch8.pdf
- [OW13] A. Obizhaeva, J. Wang. *Optimal Trading Strategy and Supply/Demand Dynamics.* J. Fin. Markets 16(1):1–32 (2013); preprint https://web.mit.edu/wangj/www/pap/OW_060408.pdf
- [SK01] A. H. Sayed, T. Kailath. *A Survey of Spectral Factorization Methods.* Numerical Linear Algebra with Applications 8 (2001). https://asl.epfl.ch/wp-content/uploads/publications/journal_articles/nlaa_2001.pdf
- [Var00] A. Varga. *Computation of inner-outer and spectral factorizations.* IEEE Trans. Automat. Control 45 (2000). https://elib.dlr.de/3506/1/varga_ieeetac2000p2.pdf
- [ZDG96] K. Zhou, J. C. Doyle, K. Glover. *Robust and Optimal Control.* Prentice Hall (1996). Standard textbook; canonical reference for inner-outer factorisation via stabilising ARE.

**Internal references (this workspace):**
- Companion paper draft: `papers/noisy-signal-impact-trading.md`
- Companion connections note: `outputs/trading-duality-extensions.md`
- Numerical experiment confirming routes (a) and (b) agree to machine precision: `experiments/closed_form_vs_operator.py`, results in `experiments/results/closed_form_vs_operator.out`.
