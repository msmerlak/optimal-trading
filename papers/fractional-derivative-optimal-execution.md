...
**Status:** Skeleton draft. Proofs in body and Appendix A; load-bearing technical lemmas flagged with `⚠️ + TODO`. The whole-line bulk theorem is verified numerically in Appendix F.

**Version:** v3 (2026-06-30).
...
The bulk term is intrinsic: it is the inverse of the propagator symbol $|\xi|^{\gamma-1}$ in Fourier on $\mathbb{R}$, restricted to the adapted subspace by the optional projection $P_+$. The translation-invariant **filtration Wiener–Hopf factorization** of the projected operator $(P_+CP_+)^{-1}$ uses the half-order factorization $|\xi|^{1-\gamma} = (i\xi)^\beta(-i\xi)^\beta$ with $\beta = (1-\gamma)/2$, giving the explicit form

$$ u^{\rm bulk}_t \;=\; \kappa_{1-\gamma}\,D_+^{\beta}\,P_+\,D_-^{\beta}\,(\alpha-\lambda)\,(t) \;=\; \kappa_{1-\gamma}\,\bigl(D_+^{\beta}\,\zeta\bigr)(t),\qquad \zeta_s := \bigl(D_-^{\beta}\bar\alpha(s,\cdot)\bigr)(s), $$

with $\kappa_{1-\gamma} = 1/[2c\,\Gamma(1-\gamma)\sin(\pi\gamma/2)]$. The intermediate $\zeta_s$ is the $\mathcal{F}_s$-adapted anticausal half-derivative of the time-$s$ forecast curve at the current time; the outer causal half-derivative $D_+^\beta$ acts on the diagonal trace $\{\zeta_s\}_{s\le t}$ along the conditioning-time axis.
...
- **Bounded-interval interpretation $\mathbb{T}=[0,T]$.** $\alpha_t = \mathbb{E}_t[P_T - P_t]$, the cumulative expected price change to the terminal horizon. This forces $\alpha_T \equiv 0$, which is consistent with the boundary condition $\bar\alpha(t,T)\to 0$ as $t\to T$ used in the bounded-interval boundary correction (§5.2).
...
**Why a naive Fourier inversion fails.** The deterministic bulk FOC $(\star_{\rm bulk})$ inverts on $\mathbb{R}$ to $u^{\rm det}_t = c_\gamma^{-1}\mathbb{D}^{1-\gamma}(\alpha-\lambda)(t)$, with $c_\gamma = 2c\,\Gamma(1-\gamma)\sin(\pi\gamma/2)$ from $\hat G(\xi)=c_\gamma|\xi|^{\gamma-1}$ (§3.1, §4.1). This $u^{\rm det}$ is non-causal in $\alpha$ — the symmetric Riesz operator $\mathbb{D}^{1-\gamma}$ samples $\alpha$ on the full line — so it is not an admissible adapted control. Replacing the realized path $\alpha$ by the forecast curve $\bar\alpha(t,\cdot)$ does restore adaptedness, but the resulting control
$$ \tilde u_t \;:=\; c_\gamma^{-1}\,\mathbb{D}^{1-\gamma}\bar\alpha(t,\cdot)(t) $$
is only the *certainty-equivalent* answer and does **not** satisfy the conditioned FOC $(\star_{\rm bulk}^{\mathcal{F}})$. Classical LQ certainty equivalence (Bensoussan 1992; Kwakernaak–Sivan 1972) requires either pointwise/Markov cost or a causal cost operator, neither of which holds here: $C = G\ast$ is non-local and its inverse $\mathbb{D}^{1-\gamma}$ is non-causal, so on the adapted subspace $(P_+CP_+)^{-1}\neq P_+C^{-1}P_+$. The correct adapted inverse is given by the filtration Wiener–Hopf factorization of the projected operator.

**Candidate policy via filtration Wiener–Hopf factorization.** On Schwartz functions the bulk symbol factorizes as

$$ |\xi|^{1-\gamma} \;=\; (i\xi)^\beta(-i\xi)^\beta,\qquad \beta := (1-\gamma)/2, $$

with $(i\xi)^\beta$ and $(-i\xi)^\beta$ extending analytically and non-vanishingly to the upper and lower complex half-planes respectively — the defining property of a Wiener–Hopf factorization (Wiener 1949; Noble 1958; SKM 1993 §7.1). The corresponding time-domain factors are the causal and anticausal Marchaud derivatives $D_+^\beta,\,D_-^\beta$ (§3.2), and $\mathbb{D}^{1-\gamma}=D_+^\beta D_-^\beta$ on $\mathcal{S}(\mathbb R)$ (SKM 1993 §7.1; expanded in §4.3 below). Correspondingly the cost operator $C$ factorizes as $C = C_-\,C_+$ with $C_\pm = c_\gamma^{1/2}I_\pm^\beta$ (Riemann–Liouville fractional integrals of order $\beta$), so $C_\pm^{-1} = c_\gamma^{-1/2}D_\pm^\beta$.

With $P_+$ the optional projection from $L^2(\mathbb R)$ onto adapted $L^2$ processes, the classical filtration Wiener filter (Wiener 1949; Bode–Shannon 1950; Kailath–Sayed–Hassibi 2000 ch. 7–8) gives
$$ (P_+CP_+)^{-1} \;=\; C_+^{-1}\,P_+\,C_-^{-1} \;=\; c_\gamma^{-1}\,D_+^\beta\,P_+\,D_-^\beta. $$
**The optional projection sits *between* the two half-order factors**, not outside both. Acting on the realized $\alpha$, this produces the candidate
$$ u^{\rm cand} \;:=\; c_\gamma^{-1}\,D_+^\beta\,P_+\,D_-^\beta\,(\alpha-\lambda) \;=\; c_\gamma^{-1}\,D_+^\beta\,\zeta, \tag{$\sharp$} $$
where the intermediate $\zeta_s := (P_+D_-^\beta\alpha)(s)$ is the $\mathcal{F}_s$-adapted realization of the anticausal half-derivative of $\alpha$. Since $D_-^\beta:H^\beta(\mathbb R)\to L^2(\mathbb R)$ is a deterministic bounded linear operator on the $s$-variable, and conditional expectation commutes with bounded linear operators acting on a separate variable (Bochner integration; Hytönen–van Neerven–Veraar–Weis 2016, Prop. 2.6.13),
$$ \zeta_s \;=\; \mathbb{E}_s[D_-^\beta\alpha(s)] \;=\; \bigl(D_-^\beta\bar\alpha(s,\cdot)\bigr)(s), \tag{$\sharp\sharp$} $$
the anticausal half-derivative of the time-$s$ forecast curve, evaluated at the current time. Adaptedness of $u^{\rm cand}_t$ at $t$ is immediate: $D_+^\beta$ at time $t$ depends only on $\{\zeta_s\}_{s\le t}$, and each $\zeta_s\in\mathcal{F}_s\subset\mathcal{F}_t$.

**The forecast curve thus appears only inside the inner step** ($\zeta_s$ at each time $s$), not as the operand of an outer Riesz applied to a frozen time-$t$ cross-section. This is the structural distinction between $u^{\rm cand}$ and the certainty-equivalent $\tilde u$.

**Verification that $(\sharp)$ solves the conditioned FOC.** Compute $\mathbb{E}_t[\zeta_s]$ for all $s$:
- $s\le t$: $\zeta_s\in\mathcal{F}_s\subset\mathcal{F}_t$, so $\mathbb{E}_t[\zeta_s]=\zeta_s$.
- $s>t$: by Bochner commutation and the forecast tower lemma, $\mathbb{E}_t[\zeta_s] = \mathbb{E}_t[D_-^\beta\bar\alpha(s,\cdot)(s)] = D_-^\beta\mathbb{E}_t[\bar\alpha(s,\cdot)](s) = D_-^\beta\bar\alpha(t,\cdot)(s)$.

Write $\hat\zeta_t(s) := \mathbb{E}_t[\zeta_s]$. By Bochner commutation of $\mathbb{E}_t$ with the deterministic $D_+^\beta$ in $s$,
$$ \mathbb{E}_t[u^{\rm cand}_v] \;=\; c_\gamma^{-1}(D_+^\beta\hat\zeta_t)(v),\qquad v\in\mathbb R. $$
Substituting into the LHS of $(\star_{\rm bulk}^{\mathcal{F}})$,
$$ \mathbb{E}_t\!\left[\int_{\mathbb R} G(|t-v|)\,u^{\rm cand}_v\,dv\right] \;=\; c_\gamma^{-1}\bigl(G\ast D_+^\beta\hat\zeta_t\bigr)(t). $$
The Fourier symbol identity
$$ \hat G(\xi)\cdot(i\xi)^\beta \;=\; c_\gamma|\xi|^{\gamma-1}(i\xi)^\beta \;=\; c_\gamma(-i\xi)^{-\beta} $$
(using $\gamma-1+\beta = -\beta$ and the phase cancellation in (4.3.1)) gives $G\ast D_+^\beta = c_\gamma\,I_-^\beta$, the anticausal Riemann–Liouville fractional integral of order $\beta$. Therefore
$$ \mathbb{E}_t[(Cu^{\rm cand})(t)] \;=\; (I_-^\beta\hat\zeta_t)(t). $$
$I_-^\beta$ at $t$ uses only $\hat\zeta_t(s)$ for $s\ge t$, where the second bullet gives $\hat\zeta_t(s) = D_-^\beta\bar\alpha(t,\cdot)(s)$. Hence
$$ (I_-^\beta\hat\zeta_t)(t) \;=\; \bigl(I_-^\beta D_-^\beta\bar\alpha(t,\cdot)\bigr)(t) \;=\; \bar\alpha(t,t) \;=\; \alpha_t, $$
using $I_-^\beta D_-^\beta=\text{id}$ on $H^\beta(\mathbb R)$ (SKM 1993 §5.3 Thm 5.3). The conditioned FOC is satisfied (modulo the DC offset $\lambda$, which contributes only to the suppressed $\xi=0$ mode under the spectral integrability hypothesis of Theorem 4.1).

**The past leg never enters.** The Plancherel collapse above samples $\hat\zeta_t(s)$ only for $s\ge t$, i.e. only on the leg where Bochner + tower gives a clean closed form. The past leg $s<t$, where $\hat\zeta_t(s) = \zeta_s$ is the genuinely $\mathcal{F}_s$-adapted intermediate (in general different from the cross-section $D_-^\beta\bar\alpha(t,\cdot)(s)$ of the time-$t$ forecast curve), drops out of the FOC verification. This is the structural reason the W-H form $(\sharp)$ closes the conditioned FOC while the certainty-equivalent $\tilde u$ does not.

Strict convexity of $\mathcal{C}$ on $L^2_{\rm adap}$ (the kernel symbol $\hat G(\xi)=c_\gamma|\xi|^{\gamma-1}$ is non-negative; positive off $\xi=0$) makes the conditioned FOC both necessary and sufficient, so $u^{\rm bulk}_t = u^{\rm cand}_t$ is the unique adapted optimum.

The bounded-interval and half-line specializations of §5 follow by the same template with $(\star^{\mathcal{F}})$ and $(\star_{\rm WH}^{\mathcal{F}})$ replacing $(\star_{\rm bulk}^{\mathcal{F}})$ and the domain of integration restricted; the boundary corrections of §5 act on the kernel of the bulk operator. Adaptedness of the resulting policy is recapped once, in §4.2.
...
**Theorem 4.1 (Bulk theorem).** *Let $G(t) = c\,t^{-\gamma}$ with $\gamma\in(0,1)$, $c > 0$. Let $\alpha$ be a stationary mean-zero $\mathcal{F}_t$-progressive process with finite power spectral density $S_\alpha(\xi)$ satisfying $\int (1+|\xi|^{2(1-\gamma)}) S_\alpha(\xi)\,d\xi < \infty$. Let $\lambda\in\mathbb{R}$ be a constant DC offset (zero for zero-mean $\alpha$). Set $\beta := (1-\gamma)/2$ and $c_\gamma := 2c\,\Gamma(1-\gamma)\sin(\pi\gamma/2)$, $\kappa_{1-\gamma}:=c_\gamma^{-1}$. Define the $\mathcal{F}_s$-adapted process*
$$ \zeta_s \;:=\; \bigl(D_-^{\beta}\,\bar\alpha(s,\cdot)\bigr)(s), \qquad s\in\mathbb R. $$
*Then the unique adapted $L^2$ minimizer of the stationary signal-adaptive cost $\mathcal{C}$ satisfying the conditioned bulk FOC $(\star_{\rm bulk}^{\mathcal{F}})$ is*

$$ \boxed{\; u^{\rm bulk}_t \;=\; \kappa_{1-\gamma}\,\bigl(D_+^{\beta}\,\zeta\bigr)(t),\qquad \beta = \tfrac{1-\gamma}{2},\qquad \kappa_{1-\gamma} = \frac{1}{2c\,\Gamma(1-\gamma)\sin(\pi\gamma/2)}.\;} $$

*Equivalently, with $P_+$ the optional projection onto $\{\mathcal{F}_s\}_{s\in\mathbb R}$,*
$$ u^{\rm bulk} \;=\; \kappa_{1-\gamma}\,D_+^{\beta}\,P_+\,D_-^{\beta}\,(\alpha-\lambda), $$
*the filtration Wiener–Hopf form $C_+^{-1}P_+C_-^{-1}$ of the projected bulk operator $(P_+CP_+)^{-1}$. The right-hand side is $\mathcal{F}_t$-measurable through $\zeta_s\in\mathcal{F}_s$ for $s\le t$ (§2.3, §4.2).*

**Remark 4.1.1 (attribution).** The structural content of Theorem 4.1 — that the optimal control for a propagator $G\propto t^{-\gamma}$ is given by half-order fractional operators of order $\beta=(1-\gamma)/2$ — is, in operator language and on the bounded interval, in Forde–Sánchez-Betancourt–Smith (2022) Theorem 2.2 proof: they factorize the Fredholm operator $T = B^{-1}I_\nu B$ with $I_\nu$ the Riemann–Liouville operator of order $r=\beta$ and write $I_\nu^{-1} = \Gamma(1-r)D^r$. Their weight conjugation by $B$ (multiplication by $t^{-(1-\gamma)/4}$) implements, on a bounded interval, both the time-domain restriction and the filtration projection $P_+$ that appears explicitly in the present W-H form. On the translation-invariant whole line the projection is the optional projection of stochastic analysis, and the answer takes the filtration-W-H form above. The bulk-symbol Wiener–Hopf factorization $|\xi|^{1-\gamma} = (i\xi)^\beta(-i\xi)^\beta$ (§4.3) is classical (SKM 1993; Krein 1962; Noble 1958); the closer prior art for the **projected** form is the classical filtration Wiener filter (Wiener 1949; Bode–Shannon 1950; Kailath–Sayed–Hassibi 2000 ch. 7–8) lifted to fractional kernels (Yaglom 1962; Hannan 1970 ch. VII). Abi Jaber–Neuman (2022) and Abi Jaber–Neuman–Tuschmann (2024) obtain the projected-operator inverse implicitly via Volterra Riccati equations.

**Proof.** Adaptedness, FOC verification, and uniqueness are derived in §2.3 around equations $(\sharp)$–$(\sharp\sharp)$, using (i) Bochner commutation of $\mathbb{E}_t$ with the deterministic bounded operators $D_\pm^{\beta}$ acting on the $s$-variable (Hytönen–van Neerven–Veraar–Weis 2016, Prop. 2.6.13), (ii) the forecast tower lemma of §2.3, (iii) the symbol identity $\hat G(\xi)(i\xi)^{\beta} = c_\gamma(-i\xi)^{-\beta}$ giving $G\ast D_+^{\beta} = c_\gamma I_-^{\beta}$, and (iv) the inverse relation $I_-^{\beta}D_-^{\beta}=\text{id}$ on $H^{\beta}(\mathbb R)$ (SKM 1993 §5.3 Thm 5.3). Strict convexity of $\mathcal{C}$ on $L^2_{\rm adap}$ (the symbol $\hat G(\xi)=c_\gamma|\xi|^{\gamma-1}$ is positive on $\xi\neq 0$) gives uniqueness. The integrability assumption $\int(1+|\xi|^{2(1-\gamma)})S_\alpha(\xi)\,d\xi<\infty$ ensures $|\xi|^{1-\gamma}\widehat{\bar\alpha}\in L^2$ pathwise, so $u^{\rm bulk}\in L^2_{\rm adap}$ by Plancherel. The Fourier-side bookkeeping is recorded in Appendix A.1. $\blacksquare$

### 4.2 Adaptedness and the forecast curve

As derived in §2.3, the unique adapted solution of the conditioned FOC $(\star_{\rm bulk}^{\mathcal{F}})$ is
$$ u^{\rm bulk}_t \;=\; \kappa_{1-\gamma}\,D_+^{\beta}\,P_+\,D_-^{\beta}\,(\alpha-\lambda)\,(t) \;=\; \kappa_{1-\gamma}\,(D_+^{\beta}\zeta)(t), $$
with $P_+$ trapped *between* the two half-order factors and $\zeta_s = D_-^{\beta}\bar\alpha(s,\cdot)(s)$ the $\mathcal{F}_s$-adapted intermediate. This subsection records two remaining facts.

*Why the projection is between the half-derivatives, not after both.* The cost operator $C = G\ast$ has non-causal symbol $\hat G(\xi) = c_\gamma|\xi|^{\gamma-1}$, factorized as $\hat G(\xi) = \hat C_-(\xi)\hat C_+(\xi)$ with $\hat C_\pm(\xi) = c_\gamma^{1/2}(\pm i\xi)^{-\beta}$ (causal/anticausal half-order integrals; $C_\pm = c_\gamma^{1/2}I_\pm^\beta$ in time domain). The projected operator on the adapted subspace is $P_+CP_+$, whose inverse is $C_+^{-1}P_+C_-^{-1}$ by classical filtration Wiener–Hopf (Wiener 1949; Bode–Shannon 1950; Kailath–Sayed–Hassibi 2000 ch. 7–8). The optional projection $P_+$ enforces adaptedness of the **intermediate** $\zeta_s = (P_+D_-^{\beta}\alpha)(s) = D_-^{\beta}\bar\alpha(s,\cdot)(s)$; the forecast curve $\bar\alpha(s,\cdot)$ enters only inside this inner anticausal step, and only at the current time $s$.

*Implementation.* Only a model of the conditional law of $\alpha$ — e.g. an OU drift, a Volterra–Gaussian process, or any forecastable $\mathcal{F}_t$-Markov structure — is needed to evaluate $\zeta_s$. For an OU signal $d\alpha_t = -\theta\alpha_t\,dt + \sigma\,dW_t$, $\bar\alpha(t,s) = e^{-\theta(s-t)}\alpha_t$ for $s > t$, and (4.3.5) below gives $\zeta_s = \theta^{\beta}\alpha_s$ pathwise, so $u^{\rm bulk}_t = \kappa_{1-\gamma}\theta^{\beta}(D_+^{\beta}\alpha)(t)$ — a single causal half-order derivative of the realized path, with a mean-reversion prefactor.

This adaptedness discussion *will not be repeated in §5*: the boundary corrections inherit the same filtration-W-H construction via $(\star^{\mathcal{F}})$ and $(\star_{\rm WH}^{\mathcal{F}})$.
...
with **no extra constant** (the half-plane phases cancel exactly) and the factors commuting on Schwartz functions. The bulk policy of Theorem 4.1 takes the **filtration-W-H form** with the optional projection $P_+$ between the two half-order factors:

$$ \boxed{\; u^{\rm bulk}_t \;=\; \kappa_{1-\gamma}\,D_+^{(1-\gamma)/2}\!\bigl[P_+\,D_-^{(1-\gamma)/2}\,\alpha\bigr](t) \;=\; \kappa_{1-\gamma}\,\bigl(D_+^{\beta}\,\zeta\bigr)(t),\;} \tag{4.3.3} $$

where $\zeta_s = (D_-^{\beta}\bar\alpha(s,\cdot))(s) = \mathbb{E}_s[D_-^{\beta}\alpha(s)]$ is the $\mathcal{F}_s$-adapted realization of the inner anticausal half-derivative (§2.3, eq. $\sharp$, and Theorem 4.1). The DC offset $\lambda$ is absorbed into $\alpha-\lambda$ in the obvious way.

**Two-step adapted realization (along the conditioning-time axis).** Read (4.3.3) as a two-step adapted computation:

1. *Anticausal factor at each time $s$.* Form $\zeta_s = D_-^{\beta}\bar\alpha(s,\cdot)(s)$ — the anticausal half-derivative of the time-$s$ forecast curve, evaluated at the current time $s$. Only the time-$s$ forecast curve is needed; $\zeta_s$ is $\mathcal{F}_s$-measurable. For Markov signals (e.g. OU) this step has a closed form (see below).
2. *Causal factor along the time series of intermediates.* Apply $D_+^{\beta}$ to the $\mathcal{F}_\cdot$-adapted process $\{\zeta_s\}_{s\le t}$ and evaluate at $t$: $u^{\rm bulk}_t = \kappa_{1-\gamma}(D_+^{\beta}\zeta)(t)$.

The non-causality of $\mathbb{D}^{1-\gamma}$ is therefore quarantined entirely in the per-time inner step (1). Step (2) is causal along the conditioning-time axis, exactly as in the classical filtration Wiener filter where causality of the realization is restored by the optional projection between the two spectral factors.
...
**The diagonal trace.** Read (4.3.3) as the diagonal trace $s\mapsto\zeta_s = D_-^{\beta}\bar\alpha(s,\cdot)(s)$ followed by the causal $D_+^{\beta}$ along the conditioning-time axis. This is the structural reason the policy is causally implementable: the anticausal half-derivative is evaluated only at the diagonal $s=s$, where the forecast curve and the realized path agree, so the dependence on the unobservable future of $\alpha$ collapses to dependence on the conditional law at time $s$ alone.
...
with full-order one-sided operators (SKM 1993 §12.1 (12.5); $\cos(\pi(1-\gamma)/2) = \sin(\pi\gamma/2)$). This identity is the symmetric Riesz operator on functions, not the projected bulk inverse on adapted processes, and is recorded here only for closed-form sanity checks. For an OU signal $d\alpha_t = -\theta\alpha_t\,dt + \sigma\,dW_t$ with forecast $\bar\alpha(t,s) = e^{-\theta(s-t)}\alpha_t$ for $s>t$, the inner anticausal half-derivative has closed form by direct Marchaud integration against the exponential tail:

$$ D_-^{\beta}\bar\alpha(t,\cdot)(t) \;=\; \theta^{\beta}\,\alpha_t, \qquad\text{equivalently}\qquad D_-^{1-\gamma}\bar\alpha(t,\cdot)(t) \;=\; \theta^{1-\gamma}\alpha_t, \tag{4.3.5} $$

so $\zeta_t = \theta^{\beta}\alpha_t$ pathwise for OU, and the bulk policy reads

$$ u^{\rm bulk,\,OU}_t \;=\; \kappa_{1-\gamma}\,\theta^{\beta}\,(D_+^{\beta}\alpha)(t),\qquad \beta = (1-\gamma)/2, \tag{4.3.6} $$

i.e. a causal half-order derivative of the realized OU path, multiplied by a mean-reversion factor $\theta^{\beta}$.
...
**Corollary 5.2 (Bounded-interval execution).** *Under the assumptions of Theorem 4.1, restricted to $\mathbb{T} = [0,T]$ with initial inventory $X_0$ and terminal constraint $X_T = 0$, the optimal trading rate is*

$$ u^*_t \;=\; \kappa_{1-\gamma}\,\mathbb{D}^{1-\gamma}_{[0,T]}\!\bigl(\bar\alpha(t,\cdot) - \lambda\bigr)(t) \;+\; \mathcal{B}_{1-\gamma}(t), \qquad t\in(0,T), $$

> **Remark.** The bounded-interval bulk operator $\mathbb{D}^{1-\gamma}_{[0,T]}$ is the Söhngen–Tricomi inverse of the symmetric Abel kernel on $[0,T]$ (§3.2). On the whole line the corresponding statement requires the optional projection $P_+$ between the two half-order factors of the bulk symbol (Theorem 4.1). The FSS2022 weight conjugation by $B$ (multiplication by $t^{-(1-\gamma)/4}$) on the bounded interval implements both the time-domain restriction and the filtration projection in one move (§6.3); confirming the identity $\mathbb{D}^{1-\gamma}_{[0,T]}\bar\alpha(t,\cdot)(t) = (C_+^{-1}P_+C_-^{-1}\alpha)(t)$ on $[0,T]$ is deferred (see TODO list at end of file).
...
- **Compact Riesz form.** The final FSS2022 formulas (e.g. their eq. (26) for rough Volterra signals) are triple integrals with incomplete-Beta and Gamma-ratio prefactors via Chakrabarti–George (1994) Abel inversion. The bounded-interval form $\kappa_{1-\gamma}\mathbb{D}^{1-\gamma}_{[0,T]}\bar\alpha(t,\cdot)(t)$ (Corollary 5.2) is more compact and is $O(N\log N)$ FFT-computable (Appendix D). On the whole line the corresponding compact form uses the filtration Wiener–Hopf factorization $\kappa_{1-\gamma}D_+^\beta P_+ D_-^\beta\alpha$ (Theorem 4.1); the bounded-interval Söhngen–Tricomi operator $\mathbb{D}^{1-\gamma}_{[0,T]}$ is conjectured to be the bounded-interval analogue of the projected W-H operator, with the boundary-weight $B$ of FSS2022 playing the role of $P_+$ (§5.2 remark).
...
**Theorem 7.1 (Matrix bulk theorem).** *Under linear impact, the vector bulk solution is the matrix–valued filtration Wiener–Hopf form*

$$ \mathbf{u}^{\rm bulk}_t \;=\; \mathbf{C}^{-1}\,\kappa_{1-\gamma}\,D_+^{\beta}\,P_+\,D_-^{\beta}\,(\bar{\boldsymbol\alpha} - \boldsymbol\lambda)\,(t) \;=\; \mathbf{C}^{-1}\,\kappa_{1-\gamma}\,(D_+^{\beta}\boldsymbol\zeta)(t),\qquad \boldsymbol\zeta_s := (D_-^{\beta}\bar{\boldsymbol\alpha}(s,\cdot))(s), $$

*with $\beta = (1-\gamma)/2$, applied componentwise after diagonalization $\mathbf{C} = Q\Lambda Q^\top$ (the projection $P_+$, the scalar half-derivatives $D_\pm^{\beta}$, and the constant matrix $\mathbf{C}^{-1}$ all commute). Equivalently, the policy decouples into $d$ scalar filtration-W-H bulk problems on the principal-component signals $Q^\top\bar{\boldsymbol\alpha}$, each solved by Theorem 4.1.*
...
**Proof.** Componentwise application of Theorem 4.1 (filtration Wiener–Hopf form) in the eigenbasis, then return to the original basis using the fact that the constant matrix $Q$ commutes with both the deterministic half-order operators $D_\pm^{\beta}$ (acting on the scalar time variable per component) and with the optional projection $P_+$ (acting componentwise on the filtration). Full argument: Appendix C. $\blacksquare$
...
A self-contained proof has been given inline in §2.3 (FOC verification of the candidate $(\sharp)$) and assembled at the theorem statement in §4.1. This appendix records the Fourier-side bookkeeping that underlies the symbol identity used inline.

**Symbol of the symmetric power-law kernel.** The Fourier transform on $\mathbb{R}$ is

$$ \hat G(\xi) \;=\; 2c\int_0^\infty t^{-\gamma}\cos(\xi t)\,dt \;=\; 2c\,\Gamma(1-\gamma)\sin(\pi\gamma/2)\,|\xi|^{\gamma-1} \;=\; c_\gamma|\xi|^{\gamma-1}, $$

using $\int_0^\infty t^{-\gamma}\cos(\xi t)\,dt = \Gamma(1-\gamma)\sin(\pi\gamma/2)|\xi|^{\gamma-1}$ for $\gamma\in(0,1)$ (Gradshteyn–Ryzhik 3.761.9; Stein 1970 §V.1; SKM 1993 §7.1) and $c_\gamma := 2c\,\Gamma(1-\gamma)\sin(\pi\gamma/2)$.

**Bulk-symbol Wiener–Hopf factorization.** With $\beta = (1-\gamma)/2$, define $(\pm i\xi)^\beta := |\xi|^\beta e^{\pm i\beta\pi\,\mathrm{sgn}(\xi)/2}$ by principal branches. Then $(i\xi)^\beta$ extends analytically and non-vanishingly to $\{\Im\xi>0\}$, $(-i\xi)^\beta$ to $\{\Im\xi<0\}$, and

$$ (i\xi)^\beta(-i\xi)^\beta \;=\; |\xi|^{2\beta}\,e^{i\beta\pi\mathrm{sgn}(\xi)/2}\,e^{-i\beta\pi\mathrm{sgn}(\xi)/2} \;=\; |\xi|^{1-\gamma}, $$

the Riesz-symbol factorization. The corresponding time-domain factors are the causal/anticausal Marchaud derivatives $\widehat{D_\pm^\beta f}(\xi) = (\pm i\xi)^\beta\hat f(\xi)$ (SKM 1993 §5.4, §7.1).

**Key symbol identity for the FOC verification.** Multiplying,

$$ \hat G(\xi)\cdot(i\xi)^\beta \;=\; c_\gamma|\xi|^{\gamma-1}|\xi|^\beta e^{i\beta\pi\mathrm{sgn}(\xi)/2} \;=\; c_\gamma|\xi|^{-\beta}e^{i\beta\pi\mathrm{sgn}(\xi)/2} \;=\; c_\gamma(-i\xi)^{-\beta}, $$

using $\gamma-1+\beta = -\beta$. The inverse Fourier transform of $c_\gamma(-i\xi)^{-\beta}\hat f(\xi)$ is $c_\gamma I_-^\beta f$ (the anticausal Riemann–Liouville fractional integral of order $\beta$). Hence on Schwartz functions

$$ G \ast D_+^\beta \;=\; c_\gamma\,I_-^\beta. \tag{A.1.1} $$

This is the identity invoked in the §2.3 FOC verification: applying $\mathbb{E}_t$ to $u^{\rm cand}_v = c_\gamma^{-1}(D_+^\beta\zeta)(v)$ and convolving with $G$ in the $v$-variable produces $(I_-^\beta\hat\zeta_t)(t)$, where $\hat\zeta_t(s) = \mathbb{E}_t[\zeta_s]$. Because $I_-^\beta$ at $t$ sees only $\hat\zeta_t(s)$ for $s\ge t$, where Bochner + tower give $\hat\zeta_t(s) = D_-^\beta\bar\alpha(t,\cdot)(s)$, and $I_-^\beta D_-^\beta = \text{id}$ on $H^\beta(\mathbb R)$ (SKM 1993 §5.3 Thm 5.3), the result equals $\bar\alpha(t,t) = \alpha_t$ and the conditioned FOC closes.

**Integrability and uniqueness.** Under the hypothesis $\int(1+|\xi|^{2(1-\gamma)})S_\alpha(\xi)\,d\xi<\infty$, $|\xi|^{1-\gamma}\hat{\bar\alpha}(\xi)\in L^2$ pathwise so $u^{\rm bulk}\in L^2_{\rm adap}$ by Plancherel. Uniqueness on $L^2_{\rm adap}$ follows from strict convexity of $\mathcal C$ (the symbol $c_\gamma|\xi|^{\gamma-1}$ is positive on $\xi\neq 0$). $\blacksquare$

#### A.2 Proof of Corollary 5.2 and Proposition 5.3

**Part 1: Corollary 5.2 via SKM 1993 §13.2 Thm 13.2.**

The bounded-interval FOC $(\star)$ is, after projection onto $\mathcal{F}_t$ (§4.2), the deterministic-in-$\mathcal{F}_t$ Fredholm equation in the forecast curve:
...
### Appendix F. Numerical verification of the bulk theorem \label{app:bulk-numerics}

This appendix records a discrete numerical experiment confirming the whole-line Theorem 4.1 on a stationary AR(1) signal with symmetric power-law impact, and quantifies the loss incurred by the naive certainty-equivalent comparator $\tilde u_t = \kappa_{1-\gamma}\mathbb{D}^{1-\gamma}\bar\alpha(t,\cdot)(t)$ (the deterministic Riesz inverse applied to the fixed time-$t$ forecast curve). Code: `experiments/bulk_vs_filtration_WH_discrete.py`.

**Setup.** Stationary AR(1) signal $\alpha_n = \rho\alpha_{n-1} + \epsilon_n$, $\epsilon_n\sim\mathcal N(0,1)$ i.i.d., $|\rho|<1$; symmetric discrete power-law kernel $G(k) = c|k|^{-\gamma}$ for $k\neq 0$ with temporary-impact diagonal $G(0) = c+\eta$; cost $J(u) = \tfrac12 u^\top G u - \alpha^\top u$ minimized over $\mathcal{F}_n$-adapted $u$. Four candidate policies are scored by $\mathrm{Sharpe}^2/T = -2J^*/T$ after analytically optimizing each candidate's scalar gain $\lambda^* = \alpha^\top s/s^\top Gs$, giving $J^*(s) = -\tfrac12(\alpha^\top s)^2/(s^\top Gs)$:

| symbol | policy shape                                                                                     |
|--------|--------------------------------------------------------------------------------------------------|
| W      | numerically optimized causal FIR of length $L$ (true LQ minimum on adapted controls; gold standard) |
| U      | Theorem 4.1: $(1-\rho)^\beta D_+^\beta\alpha_n$, with $\beta = (1-\gamma)/2$                    |
| P      | certainty-equivalent comparator $\tilde u$: $D_+^{1-\gamma}\alpha_n + (1-\rho)^{1-\gamma}\alpha_n$ |
| $\alpha$ | naive alpha-chase: $u_n = \alpha_n$                                                            |

The AR(1) closed-form reductions used: $D_-^\beta\bar\alpha(s,\cdot)(s) = (1-\rho)^\beta\alpha_s$ (discrete analog of $\theta^\beta\alpha_s$ via $\sum_{k\ge 0}(-1)^k\binom{\beta}{k}\rho^k = (1-\rho)^\beta$), and analogously $(1-\rho)^{1-\gamma}\alpha_s$ for the full-order anticausal half-sum entering P. The Wiener gold-standard W is obtained by solving the normal equations $M h^* = R$ for the optimal length-$L$ causal FIR with $M_{km} = (G*R)(k-m)$ the per-time Hessian and $R(k) = \rho^k/(1-\rho^2)$ the AR(1) autocorrelation; a small Tikhonov ridge $10^{-8}\mathrm{tr}(M)/L$ stabilizes the ill-conditioned Toeplitz solve.

**Convergence to the gold standard** ($\gamma=0.3,\;\rho=0.9,\;\eta=1$, $N=10^5$ samples, $K_{\rm hist}=\max(2000,2L)$):

| $L$    | $S^2_W$  | $S^2_U$  | $S^2_P$  | $U/W$ | $P/W$ |
|--------|----------|----------|----------|-------|-------|
...
U matches the numerically optimized causal Wiener FIR to within 0.1% — i.e. U **is** the optimum up to finite-sample noise, confirming Theorem 4.1. The certainty-equivalent comparator P loses 14% of the achievable Sharpe$^2$ in this regime.

**Scan over $(\gamma,\rho,\eta)$** ($N=5\cdot 10^4$, $L_{\rm wiener}=200$, $K_{\rm hist}=500$):

| $\gamma$ | $\rho$ | $\eta$ | $S^2_W$ | $S^2_U$ | $S^2_P$ | $U/W$ | $P/W$ |
|----------|--------|--------|---------|---------|---------|-------|-------|
...
Three empirical patterns confirm the structural analysis:

1. **U tracks W everywhere** (up to length-$L$ truncation of the gold-standard FIR), confirming the filtration-W-H form is the adapted optimum on $\mathbb R$.
2. **P is uniformly suboptimal vs U** in any non-trivial regime, with the gap largest for small $\gamma$ and moderate $\eta$; P captures only $\sim 72\%\!-\!86\%$ of the achievable Sharpe$^2$ when $\gamma=0.3$.
3. The gap shrinks toward zero only in the asymptotic limits (i) $\gamma\to 1$ (the Riesz operator becomes the identity and both forms reduce to alpha-chase) and (ii) large $\eta$ (the kernel becomes diagonal-dominated). These are the regimes where certainty equivalence becomes asymptotically valid, since the cost operator approaches a pointwise multiplier.
...
## Changelog — v3 (2026-06-30)

The whole-line bulk theorem (Theorem 4.1) is now stated and proved in **filtration Wiener–Hopf form**: with $\beta = (1-\gamma)/2$,
$$ u^{\rm bulk}_t \;=\; \kappa_{1-\gamma}\,D_+^{\beta}\,P_+\,D_-^{\beta}\,(\alpha-\lambda)\,(t) \;=\; \kappa_{1-\gamma}\,(D_+^{\beta}\zeta)(t),\qquad \zeta_s := D_-^{\beta}\bar\alpha(s,\cdot)(s), $$
with the optional projection $P_+$ between the two half-order factors of the bulk symbol. The previous closed form, $\kappa_{1-\gamma}\mathbb{D}^{1-\gamma}\bar\alpha(t,\cdot)(t)$, was the certainty-equivalent answer; it does not satisfy the conditioned FOC because the cost operator $C = G\ast$ is non-local and its inverse is non-causal, so $(P_+CP_+)^{-1}\neq P_+C^{-1}P_+$ on the adapted subspace. Numerical comparison on AR(1) signals (Appendix F) confirms that the W-H form matches the optimized causal Wiener FIR to within 0.1% while the certainty-equivalent comparator loses up to 14% of the achievable Sharpe$^2$.

**Sections rewritten.** §2.3 (candidate policy + FOC verification), §4.1 (Theorem 4.1 statement + proof), §4.2 (adaptedness via projected operator), §4.3 (W-H realization, OU specialization), §7 (multi-asset matrix form), Appendix A.1 (Fourier-side proof), Appendix F (new — numerical verification). Abstract, §4 opener, §5.2 Corollary 5.2, and §6.3 FSS2022 comparison updated for consistency.

**Open items for v4.**
- §5 boundary corrections under the projected operator $P_+CP_+$ (kernel structure expected unchanged; explicit check pending).
- §5.2 Corollary 5.2: verify the bounded-interval identity $\mathbb{D}^{1-\gamma}_{[0,T]}\bar\alpha(t,\cdot)(t) = (C_+^{-1}P_+C_-^{-1}\alpha)(t)$ via the FSS2022 weight conjugation by $B$.
- Possible relaxation of Theorem 4.1 spectral integrability hypothesis using the projected-operator domain.
- Sync `outputs/bulk-fractional-forecast-novelty.md` to the current statement.

**Audit trail.** `notes/bulk-theorem-refutation.md`, `experiments/bulk_vs_filtration_WH_discrete.py`, `papers/.plans/bulk-theorem-correction.md`.
