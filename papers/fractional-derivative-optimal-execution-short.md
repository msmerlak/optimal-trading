...
**Boundary correction (decaying W-H mode).** The one-dimensional kernel of the half-line operator is spanned by the tempered distribution $\psi_\eta$ with $\hat\psi_\eta(\xi) = C/M_+(\xi)$, $C$ fixed by $\int\psi_\eta = X_0 - \int u^{\rm bulk}$. Asymptotically: at low frequency $\hat\psi_\eta\sim (-i\xi)^{(1-\gamma)/2}$ giving a $t^{(\gamma-3)/2}$ power-law tail; at high frequency $\hat\psi_\eta\to C/\sqrt{\eta}$ giving a short-time transient of width $1/\xi_*(\eta)$ and height $\sim 1/\sqrt{\eta}$ (the smoothed block-trade). The $\eta\to 0$ limit contracts this transient to $X_0\,\delta(t)$. Full derivation and regularity caveats: long paper §5.3.5, Proposition 5.8.
