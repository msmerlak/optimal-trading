...
\begin{abstract}
A trader holding a return-predicting signal must weigh the gain a forecast promises against the cost of realizing it and the risk of holding it, acting on current information alone. The problem is a quadratic optimization: the gain of the position against the return, net of three frictions --- temporary impact, transient impact through a propagator kernel, and mean--variance inventory risk. The friction operator is non-local in time, so the unconstrained optimum needs the signal's future, and adaptedness is the binding constraint. Existing treatments characterize the adapted optimum implicitly, as the solution of an integral equation or an equivalent forward--backward system. Using a Wiener--Hopf factorization of the friction into causal and anticausal parts, we compute it in closed form away from horizon boundaries, by whitening the forecast, projecting onto the past, and recolouring. For a stationary Gaussian signal this is an explicit linear \emph{trading filter}. It reduces to the Markowitz rule under risk alone and, under pure power-law impact, to a fractional derivative of the signal of order $\nu=(1-\beta)/2$ with $\beta$ the impact exponent; rational frictions give filters with finitely many exponential moving averages, recovering the G\^arleanu--Pedersen aim portfolio and the stationary Neuman--Vo\ss{} solution. The causal factor on the imaginary axis yields a closed-form value of information, a causality gap $\sin(\pi\beta/2)$ under power-law impact, and a rate response that identifies when the optimum trades against its own transient impact. On a finite horizon the factors become terminal-anchored Gohberg--Krein factors, recovering the Gatheral--Schied--Slynko and Forde--S\'anchez-Betancourt--Smith solutions. All closed forms are checked against discretized adapted optima.
\end{abstract}
...
\end{equation}
the value attainable with the whole signal path in hand. Equation~\eqref{eq:vant} is the position-referred form, finite for $\lambda>0$; for the scale-free kernel at $\lambda=0$ the position form diverges, and the anticipative value is referred to the rate, $v_{\rm ant}=\frac{1}{4\pi}\int S_\alpha/\hat q\,d\omega$, finite there and used in Appendix~\ref{app:B} to obtain \eqref{eq:sinlaw}. The \emph{causality gap} is the ratio $v/v_{\rm ant}\le1$, the fraction of the clairvoyant value that survives the restriction to current information. The shortfall $v_{\rm ant}-v$ equals, by \eqref{eq:value}, the $L^2$ energy of the anticausal part of the half-whitened signal $h=\hat\psi\hat n_-^{-1}$, and by Remark~\ref{rmk:duality} the friction energy of the nonanticipativity multiplier $\xi^\star$ --- the shadow price of adaptedness, in closed form. For an Ornstein--Uhlenbeck signal the value $v=\sigma^2\theta/4\Phi(\theta)^2$ turns on the friction factor $\Phi(\theta)=\hat n_+(i\theta)$. 

Under pure power-law impact the causality gap is scale-free, independent of the signal speed (\S\ref{sec:fracpolicy}):
...
The proof is in Appendix~\ref{app:C}. When the signal mean-reverts faster than the impact decays, $\theta>\kappa$ at $\lambda=0$, a signal-following trade would leave an impact residual that outlives the signal, and the optimal rate instead runs against the signal to ride that residual --- \emph{impact surfing}. Risk aversion lowers the threshold, and for $\lambda\ge2\kappa\gamma/3$ the optimal rate runs against the signal at every speed: the position stays aligned with the signal, but its mean reversion dominates the conditional rate. The pure-risk limit $R=-\theta^2/\lambda$ is the extreme case, a Markowitz position always conditionally exiting. For the power-law kernel the rate and position are aligned at every speed and risk aversion; the reversal does not occur there. A temporary cost removes the reversal as well: for $\eta>0$ the atom vanishes and, at fixed return variance, the value falls with speed as $\theta^{-2}$ (\S\ref{sec:value}), the per-trade cost of fast trading. Figure~\ref{fig:surf} shows the rate response and the resulting phase diagram.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_impact_surfing}
\caption{Impact surfing (exponential kernel, $\gamma=1$, $\kappa=2$). (a)~The rate response $R(\theta)$: the exponential kernel (solid) reverses sign at $\theta^\ast$ as inventory risk $\lambda$ grows, while the power-law kernel (dashed) stays positive at every risk aversion. (b)~Phase diagram: the optimum follows the signal ($R>0$) below the boundary $\theta^\ast(\lambda)$ and trades against it ($R<0$) above; the boundary meets the axes at $\theta=\kappa$ and $\lambda=2\kappa\gamma/3$.}
\label{fig:surf}
\end{figure}

The vanishing atom is one face of a broader regularity: the power-law optimum never trades in blocks. A block trade --- a jump in the position --- pays a transient-impact cost proportional to the kernel at zero lag, $g(0)$, infinite for $g(t)=|t|^{-\beta}$. Instantaneous trading is therefore never optimal, neither on the whole line, where $c_1=0$ removes the lag-zero atom, nor at the ends of a finite horizon, where the optimal rate diverges only as the integrable $d(t)^{-\nu}$ in the distance to the endpoint (Proposition~\ref{prop:boundary}), so the position stays continuous. The exponential kernel is the opposite: $g(0)$ is finite, a block costs only finitely, and the optimal liquidation places discrete trades at both ends --- the block-plus-continuous strategy of \citet{ObizhaevaWang2013}. These distinctions belong to the order-book resilience literature: \citet{AlfonsiSchiedSlynko2012} treat together whether an optimal execution trades in blocks, whether it stays one-signed (their \emph{positive portfolio problem}), and whether the model admits price manipulation. The first two separate the kernels --- the power law, with $g(0)=\infty$, trades in no blocks and, by the positive rate response \eqref{eq:response}, never reverses sign, while the exponential does both --- and the third is ruled out for either by the positive-definiteness $\hat g\ge0$, which also makes impact surfing a cost-optimal response rather than a manipulation \citep{Gatheral2010,AlfonsiSchiedSlynko2012}. The frictions omitted here --- spread, position limits, order discreteness --- would temper the reversal, so the prediction holds within the model class.
